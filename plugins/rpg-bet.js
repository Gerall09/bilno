...
async function handler(m, { conn, args }) {
    ...
    try {
        ...
        if (!(m.sender in confirm)) {
            ...
            let txt = `Apakah Kamu Yakin Mau Judi Sama Tinasha?\n\n*Taruhan:* ${count}\nWaktu 60 Detik\n\nBalas *Y* Jika Mau\nBalas *N* Untuk Membatalkan`
...
Kamu *${status}*, Kamu ${status == 'Win' ? `Mendapatkan *+${count * 2}*` : status == 'Lose' ? `Kehilangan *-${count * 1}*` : `Mendapatkan Uang *+${Math.floor(count / 1.5)}*`} 
  `.trim())
            clearTimeout(timeout)
            delete confirm[m.sender]
            return !0
        } else if (/^(Tidak|no|N)?$/i.test(txt)) {
            clearTimeout(timeout)
            delete confirm[m.sender]
            m.reply('Rejected')
            return !0
