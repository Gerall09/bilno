...
let handler = async (m) => {
    ...
    if (command[0] === '.moneykaget' && command[1] !== 'claim' && command[1] !== 'hapus' && command.length === 3) {
        ...
        if (isNaN(amount) || isNaN(numParticipants) || numParticipants < minParticipants || numParticipants > maxParticipants) {
            conn.reply(chatId, `⚠️ Format salah! Gunakan .moneykaget <money> <jumlah pengguna>. Jumlah pengguna harus antara ${minParticipants} dan ${maxParticipants}.`, m);
            return;
        }
        // Validasi cooldown
        if (userData.lastMoneykaget + moneykagetCooldown > currentTime) {
            ...
            conn.reply(chatId, `⏳ Kamu @${user.split('@')[0]}, perlu menunggu ${remainingTime} sebelum bisa membuat moneykaget lagi.`, m);
            return;
        }
        // Validasi money cukup
        if (userData.money < amount) {
            conn.reply(chatId, `⚠️ Kamu tidak memiliki cukup money (${amount} money diperlukan) untuk membuat moneykaget ini.`, m);
            return;
        }
        // Kurangi money dan buat moneykaget baru
...
🔕 Kadaluarsa Dalam 30 menit`, m);
        setTimeout(() => {
            ...
            if (activeMoneykagets.has(code)) {
                ...
                if (moneykaget.claimed.length < moneykaget.numParticipants) {
                    ...
                    conn.reply(chatId, `⏰ Moneykaget dengan code ${code} telah kedaluwarsa. Sisa uang ${remainingAmount} Money telah dikembalikan ke @${user.split('@')[0]}.`, m);
                }
    ...
    if (command[0] === '.moneykaget' && command[1] === 'claim' && command.length === 3) {
        ...
        if (moneykaget.claimed.includes(user)) {
            conn.reply(chatId, `⚠️ Kamu sudah mengambil moneykaget ini.`, m);
            return;
        }
        if (moneykaget.claimed.length >= moneykaget.numParticipants) {
        ...
        conn.reply(chatId, `🎉 @${user.split('@')[0]} telah menerima ${share} Money!`, m);
        if (moneykaget.claimed.length === moneykaget.numParticipants) {
    ...
    if (command[0] === '.moneykaget' && command[1] === 'hapus' && command.length === 3) {
        ...
        if (moneykaget.creator !== user) {
            conn.reply(chatId, `⚠️ Kamu tidak memiliki izin untuk menghapus moneykaget ini.`, m);
            return;
        }
        let remainingAmount = moneykaget.amount - calculateTotalClaimed(moneykaget.claimed, moneykaget.amount, moneykaget.numParticipants);
        userData.money += remainingAmount;
        activeMoneykagets.delete(code);
        conn.reply(chatId, `🗑️ Moneykaget dengan code ${code} telah dihapus oleh @${user.split('@')[0]}. Sisa uang ${remainingAmount} Money telah dikembalikan.`, m);
    }
